...
提示词 [`prompts/perception.jinja2`](../prompts/perception.jinja2) 明确禁止 LLM 返回 `route`、`clarification`、`question`。这些是规则层职责。
...
实现：[`agent/graph.py`](../agent/graph.py) 的 `_empathy_agent()`；话术提示词：[`prompts/empathy.jinja2`](../prompts/empathy.jinja2)。
