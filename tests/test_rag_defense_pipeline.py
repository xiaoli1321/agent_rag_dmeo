...
def _doc(
    text: str, *, score: float = 0.82, title: str = "CGM 参数说明"
) -> RetrievedDoc:
    return RetrievedDoc(
        source_title=title,
...
def test_c1_grader_blocks_high_score_but_unrelated_parameter_doc() -> None:
    ...
    assert (
        result.debug_trace["document_grades"][0]["failure_type"] == "retrieval_mismatch"
    )
...
def test_c1_marks_empty_retrieval_as_knowledge_missing() -> None:
    ...
    result = StubRagService(settings=_service().settings).answer(
        "火星真空环境能不能佩戴？"
    )
