...
class DemoSettings(BaseSettings):
    ...
    def normalize(self) -> "DemoSettings":
        ...
        self.qdrant_collection = self.qdrant_collection.strip() or "cgm_customer_support"
        ...
        self.embedding_api_base = self._clean(self.embedding_api_base) or self.qwen_api_base
        self.embedding_api_key = self._clean(self.embedding_api_key) or self.qwen_api_key
        self.embedding_model = self._clean(self.embedding_model) or self.qwen_embedding_model or "qwen3-vl-embedding"
        ...
        self.agent_retrieval_strategy = (self.agent_retrieval_strategy or "hybrid").strip().lower()
        if self.agent_retrieval_strategy not in {"dense", "hybrid"}:
            self.agent_retrieval_strategy = "hybrid"
        self.demo_chunking_strategy = (self.demo_chunking_strategy or "structural").strip().lower()
        if self.demo_chunking_strategy not in {"recursive", "structural", "parent-child"}:
    ...
    def embedding_configured(self) -> bool:
        return bool(self.embedding_api_base and self.embedding_api_key and self.embedding_model)
