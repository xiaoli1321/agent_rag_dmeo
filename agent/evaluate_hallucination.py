...
HIGH_RISK_NUMBER_PATTERN = re.compile(r"(?<![\w.])\d+(?:\.\d+)?\s*(?:位|天|小时|分钟|米|feet|ft|级|%)?", re.IGNORECASE)
...
def evaluate_case(case: EvalCase, result: dict) -> dict:
    ...
    source_titles = [str(doc.get("source_title") or "") for doc in retrieved_docs if isinstance(doc, dict)]
    source_text = "\n".join(source_titles)
    should_refuse = not case.should_answer

    refused = answer_status == "insufficient_evidence" or "没有在当前知识库找到足够依据" in answer or "已为你转人工" in answer
    has_reference = "引用：" in answer
    expected_source_hit = (
        not case.expected_sources
        or any(expected.lower() in source_text.lower() for expected in case.expected_sources)
        or any(expected.lower() in answer.lower() for expected in case.expected_sources)
    )
    expected_terms_hit = all(term.lower() in answer.lower() for term in case.expected_terms)
    risky_numbers = _risky_numbers(answer) if should_refuse else []
    hallucination = (should_refuse and not refused) or (should_refuse and bool(risky_numbers)) or (case.should_answer and not refused and not expected_source_hit)
            ...
            "{expected_source_hit} | {risky_numbers} | {hallucination} |".format(
                **{
                    **row,
                    ...
                    "risky_numbers": ", ".join(row["risky_numbers"]) if row["risky_numbers"] else "",
                }
            )
        )
...
def _risky_numbers(answer: str) -> list[str]:
    return [match.group(0).strip() for match in HIGH_RISK_NUMBER_PATTERN.finditer(answer)]


def main() -> None:
    parser = argparse.ArgumentParser(description="Run CGM Agent hallucination evaluation.")
    ...
    output_path = Path(args.output) if args.output else DEFAULT_RUNS_DIR / f"hallucination_eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
